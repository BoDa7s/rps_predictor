...
export default function PredictionSourcesPanel({
  ...
  return (
    <section className="min-h-0 min-w-0">
      <div className="mb-[clamp(0.16rem,0.1rem+0.14vh,0.28rem)] flex items-center justify-between gap-[clamp(0.25rem,0.16rem+0.2vw,0.45rem)]">
      ...
      <div className={layout === "rows" ? "min-w-0 divide-y divide-[color:var(--app-border)]" : "grid gap-2 sm:grid-cols-2 md:grid-cols-1 lg:grid-cols-2"}>
        {visibleSources.map(source => {
          ...
          return (
            ...
            >
              <div className={`flex min-w-0 items-center justify-between gap-[clamp(0.2rem,0.14rem+0.14vw,0.34rem)] ${isTightDensity ? cramped ? "text-[0.62rem]" : "text-[clamp(0.62rem,0.56rem+0.14vw,0.72rem)]" : "text-[clamp(0.72rem,0.64rem+0.2vw,0.85rem)]"}`}>
                <span className="min-w-0 break-words font-medium text-[color:var(--app-text-primary)]">{source.label}</span>
                <span className={`min-w-0 break-words text-right font-semibold uppercase tracking-[0.14em] text-[color:var(--app-accent-strong)] ${isTightDensity ? cramped ? "text-[0.5rem]" : "text-[clamp(0.46rem,0.42rem+0.1vw,0.56rem)]" : "text-[clamp(0.54rem,0.48rem+0.12vw,0.68rem)]"}`}>
                  {source.value}
                </span>
              </div>
              <div className={`mt-[clamp(0.12rem,0.06rem+0.08vh,0.2rem)] overflow-hidden rounded-full bg-[color:var(--app-surface-input)] ${isTightDensity ? cramped ? "h-[0.14rem]" : "h-[0.18rem]" : isCompactDensity ? "h-[0.22rem]" : "h-[clamp(0.22rem,0.18rem+0.16vh,0.38rem)]"}`}>
