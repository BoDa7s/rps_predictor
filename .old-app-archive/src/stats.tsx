...
import type { RoundRecord, StatsProfileRecord } from "./lib/dataContracts";
  ...
  try {
    ...
    return parsed.map((item: StatsProfile | any) => {
      ...
      const trainingCompleted =
        item?.trainingCompleted !== undefined
          ? Boolean(item.trainingCompleted)
          : Boolean(item?.trained);
      return {
...
function toStatsProfileFromCloud(record: StatsProfileRecord, playerId: string): StatsProfile {
  const baseName = normalizeBaseName(record.base_name ?? record.display_name);
  const version = Number.isFinite(record.profile_version)
    ? Math.max(1, Math.floor(record.profile_version))
    : 1;
  const name = record.display_name || makeProfileDisplayName(baseName, version);
  return {
    id: record.id,
    playerId,
    baseName,
    ...
    trainingCount: record.training_count ?? 0,
    trainingCompleted: Boolean(record.training_completed),
    predictorDefault: Boolean(record.predictor_default),
    seenPostTrainingCTA: Boolean(record.seen_post_training_cta),
    previousProfileId: record.previous_profile_id,
    nextProfileId: record.next_profile_id,
  } satisfies StatsProfile;
}

function toRoundLogFromCloud(record: RoundRecord, playerId: string): RoundLog {
  const normalizedBestOf = (() => {
    const value = Number(record.best_of);
    return value === 5 || value === 7 ? (value as BestOf) : 3;
  })();
  const bucketValue = record.confidence_bucket;
  const normalizedBucket: "low" | "medium" | "high" = bucketValue === "high" ? "high" : bucketValue === "medium" ? "medium" : "low";
  return {
    id: record.id,
    sessionId: record.session_id ?? "",
    matchId: record.match_id ?? undefined,
    playerId,
    profileId: record.stats_profile_id,
    t: record.played_at,
    mode: record.mode as Mode,
    bestOf: normalizedBestOf,
    difficulty: record.difficulty as AIMode,
    player: record.player_move as Move,
    ai: record.ai_move as Move,
    outcome: record.outcome as Outcome,
    policy: record.decision_policy as DecisionPolicy,
    mixer: undefined,
    heuristic: undefined,
    streakAI: 0,
    streakYou: 0,
    ...
    confidenceBucket: normalizedBucket,
    decisionTimeMs: undefined,
  } satisfies RoundLog;
}
...
export function StatsProvider({ children }: { children: React.ReactNode }) {
  ...
  const {
    status: cloudStatus,
    statsProfiles: cloudStatsProfiles,
    primaryProfile: cloudPrimaryProfile,
    rounds: cloudRounds,
  ...
  useEffect(() => {
    ...
    if (cloudStatus === "hydrated" || cloudStatus === "hydrating") {
      return;
    }
  ...
  }, [cloudStatus, currentPlayerId, currentProfileId, playerProfiles]);
  ...
  useEffect(() => {
    if (cloudStatus !== "hydrated") return;
    const resolvedProfile = cloudPrimaryProfile ?? null;
    if (!resolvedProfile) return;
    const resolvedPlayerId = currentPlayerId ?? resolvedProfile.user_id ?? "";
    if (!resolvedPlayerId) return;

    if (cloudSession?.id) {
      sessionIdRef.current = cloudSession.id;
    }

    if (cloudStatsProfiles.length) {
      const nextProfiles = cloudStatsProfiles.map(profileRecord =>
        toStatsProfileFromCloud(profileRecord, resolvedPlayerId),
      );
      setProfiles(nextProfiles);
      setProfilesDirty(false);
      setCurrentProfileId(resolvedProfile.id);
      saveCurrentProfileId(resolvedProfile.id);
    }

    const nextRounds = cloudRounds.map(roundRecord => toRoundLogFromCloud(roundRecord, resolvedPlayerId));
