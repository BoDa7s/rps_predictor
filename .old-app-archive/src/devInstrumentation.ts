...
class DevInstrumentation {
  ...
  private readonly markerLog: Array<{ name: string; at: number; data?: Record<string, unknown> | null }> = [];
  ...
  logMarker(name: string, data?: Record<string, unknown> | null) {
    const entry = { name, at: Date.now(), data: data ?? null };
    this.markerLog.push(entry);
    if (this.markerLog.length > MAX_CLICK_HISTORY) {
      this.markerLog.shift();
    }
    // eslint-disable-next-line no-console
    console.info(`[dev][marker] ${name}`, data ?? "");
  }
...
export type DevInstrumentationApi = {
  ...
  logMarker: (name: string, data?: Record<string, unknown> | null) => void;
};
...
function createDevInstrumentation(): DevInstrumentationApi {
  ...
  const api: DevInstrumentationApi = {
    ...
    logMarker: (name, data) => instance.logMarker(name, data ?? null),
  };
