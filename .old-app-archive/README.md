...
Cloud synchronisation is behind a launch flag so it can be rolled out gradually. Add the following line to your `.env.local` (o
r the environment used for Vite) to enable Supabase-backed hydration, sessions, and stats:

```
VITE_FEATURE_CLOUD_SYNC=true
```

When the flag is omitted or set to any value other than `true`, the app falls back to the existing local-only experience.
